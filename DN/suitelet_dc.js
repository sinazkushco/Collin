var RECORD, SEARCH, LOG, CRED, HTTPS, RUNTIME, EMAIL, TASK, KUSH_UTILS;
 ...
 *@NModuleScope Public
 */
define(['N/record', 'N/search', 'N/log', '/SuiteScripts/CW/credentials2.0.js', 'N/https', 'N/runtime', 'N/email', "N/task", "/SuiteScripts/Library/kush_utilities.js"], run_suitelet);

//********************** MAIN FUNCTION **********************
function run_suitelet(record, search, log, cred, https, runtime, email, task, kush_utils) {
    RECORD = record;
    SEARCH = search;
    LOG = log;
    CRED = cred;
...
function execute(context) {
    ...
    if (context.request.method == 'GET') {
        ...
        if (type == "force_assignment") {
            var order = force_delivery_driver(context);
            send_response(order)
        }

        if(type == "unassign_delivery"){
            var assignment = unassign_delivery(context);
            send_response(assignment)
        }
    ...
    if (valid_security_token) {
        ...
        if (see_all_orders.indexOf(employee_id) > -1) {
            ...
            search_filter = [
                ...
                "AND", ["taxline", "is", "F"],
                "AND", ["shipping", "is", "F"],
                "AND",["datecreated", "after", "7/27/2018 11:59 pm"],
        ...
        item_fulfillment_search_obj.run().each(function (result) {
            ...
            var driver_notes = result.getValue({
                name: "memo",
                join: "createdFrom"
            });

            var isCtItem = result.getValue('custcol_kush_cteligible')
            log.debug('isCT', isCtItem)
            ...
            if(isCtItem){
                ctAmount = (((true_quantity * item_rate).toFixed(2)) * (.03)).toFixed(2);
            }
            ...
            var order_has_ctItems = parseFloat(ctAmount) > 0 ? true : false;

            var total = (true_quantity * (parseFloat(item_rate) + parseFloat(tax_rate)) + parseFloat(ctAmount)).toFixed(2);
            ...
            if (!order_obj[item_fulfillment_number]) {
                order_obj[item_fulfillment_number] = {
                    ...
                    order_has_ctItems: order_has_ctItems,
                    items: [{
                        name: item_name,
...
function validate_login(context) {
    ...
    log.debug('matt employee search results length: ', employee_search_results.length);
...
//         type: type
//     };
//     var environmentId = RUNTIME.accountId;
   

//     var my_request = {};
//     my_request.headers = {};
//     my_request.headers.Authorization = 'NLAuth nlauth_account=' + RUNTIME.accountId + ',nlauth_email=' + CRED.email + ',nlauth_signature=' + CRED.signature + ',nlauth_role=' + CRED.role;
//     my_request.headers["content-type"] = 'application/json';
//     // my_request.url = 'https://4516274-sb3.restlets.api.netsuite.com/app/site/hosting/restlet.nl?script=557&deploy=1' //WMS RESTlet URL
...
//     // my_request.testurl = "https://"+environmentId+".restlets.api.netsuite.com/app/site/hosting/restlet.nl?script=557&deploy=1"; //Production RESTlet URL
...
//     // log.audit("TEST URL: ", my_request.testurl)

//     // my_request.testurl = KUSH_UTILS.getScriptUrl({
//     //     deploymentId: 'customdeploy_restlet_dc_deployment',
//     //     scriptId: 'customscript_restlet_dc',  
//     // })
//     // log.audit("url audit: ",JSON.stringify({
//     //     'RUNTIME: ' : RUNTIME.accountId,
//     //     'URL: ' : my_request.testurl,
//     //     'ROLE: ': CRED.role
//     // }));
...
function crypt(context, type, string) {
    ...
    my_request.url = "https://"+RUNTIME.accountId+".restlets.api.netsuite.com/app/site/hosting/restlet.nl?script=557&deploy=1"
    my_request.body = JSON.stringify(request_obj);
    var my_response = HTTPS.post(my_request);
    var hashed_string = JSON.parse(my_response.body).hashed;
...
function obtain_registration_url(context) {
    ...
    if (employee_id) {
        var url = "https://www.kushsupplyco.com/delivery_confirmation/set_password.html";
...
function placeholder_orders() {
    var orders_array = {
        "FUL41312": {
            ...
            "location": "6",
            "date": "3/29/2018",
            "driver_notes": "This is a test note. Please delivery near rear entrance.",
            "items": [{
...
function force_delivery_driver(context) {
    var fulfillment_id = context.request.parameters.fulfillment_id.replace(/\D/g, '');
    var employee_id = context.request.parameters.employee_id;
    var deliveryDriver;
    var orderDetails;
    ...
    itemfulfillmentSearchObj.run().each(function (result) {
        ...
        deliveryDriver = result.getValue({
            name: "entityid",
            join: "CUSTBODY_DELIVERYDRIVER"
        });
    ...
    if(internalID){
        RECORD.submitFields({
            type: 'itemfulfillment',
            id: internalID,
            values: { 'custbody_deliverydriver': employee_id }
        });
        return true;
    }
    return false;
...
function setDeliveryDriver(context) {
    var employee_id = context.request.parameters.employee_id;
    var fulfillment_id = context.request.parameters.fulfillment_id.replace(/\D/g, '');
    var fulfillmentAlreadyAssigned = false;
    var internalID;
    var deliveryDriver;
    ...
    if(searchResultCount < 1){
        ...
        return {
            status: 'failed',
            message: 'No item fulfillment found'
        }
    }
    ...
    if (deliveryDriver) {
        return {
            ...
            message: 'This delivery is already assigned to ' + deliveryDriver
        }
    }
...
/*========================================================================================================================================*/
function unassign_delivery(context){
    log.audit("CONTEXT", context)

    var id = context.request.parameters.fulfillment_id.replace(/\D/g, '');
    var deliveryDriver;
    ...
    if (deliveryDriver) {
        RECORD.submitFields({
            type: 'itemfulfillment',
            id: id,
            values: { 'custbody_deliverydriver': "" }
        })
