...
define(["N/ui/serverWidget", "N/record", "N/redirect", "N/search"], function (serverWidget, record, redirect, search) {
    function onRequest(context) {
        if (context.request.method === "GET") {
            ...
            var shipType = context.request.parameters.shiptype || false;
            var form = createForm(location, includeTomorrow, startDate, shipType);
    ...
    //HELPER FUNCTIONS
    function createForm(location, includeTomorrow, startDate, shipType) {
        var form = serverWidget.createForm({
            title: "Garden Grove Bin Transfer Form"
        });

        form.clientScriptModulePath = "./client_gg_bin_transfer.js"; // TODO: Confirm with production

        form.addSubmitButton({
            label: "Transfer to Picking Bin"
        });
        ...
        form.addPageLink({
            ...
            title: "Freight Bin Transfer Form",
            url: "https://system.na2.netsuite.com/app/site/hosting/scriptlet.nl?script=798&deploy=1"
        });
        ...
        var shippingTypeField = form.addField({
            id: "custpage_ship_type",
            label: "Ship Type",
            type: serverWidget.FieldType.SELECT,
            source: "350" // TODO: Make list on production
        });
        ...
        if (shipType) {
            shippingTypeField.defaultValue = shipType;
        }
    ...
    function addDataToForm(form, location, includeTomorrow, startDate, shipType) {
        var binObj = get_data(includeTomorrow, startDate, shipType);
        ...
        if (binTransferObj.items.length != 0) {
            ...
            for (var i = 0; i < binTransferObj.items.length; i++) {
                ...
                invDetail.setCurrentSublistValue({
                    sublistId: "inventoryassignment",
                    fieldId: "tobinnumber",
                    value: "1004" // TODO: Picking Bin ID - Update with Production
                });
    ...
    function get_data(todayOrTomorrow, startDate, shipType) {
        todayOrTomorrow = todayOrTomorrow ? "tomorrow" : "today";
        startDate = startDate || "today";

        var allShipMethods =  ["shipmethod", "noneof", "5382", "4774", "10694"]; // Reserved Do Not Ship , Freight, Local Pickup GG
        var expeditedShipMethods =  ["shipmethod", "anyof", "38", "39", "4776", "4775", "5230", "5266", "12523", "12524", "12525", "12526", "12527", "12528", "8142", "8255"]; // Reserved Do Not Ship , Freight, Local Pickup GG
        var groundShipMethods =  ["shipmethod", "anyof", "40", "41", "37", "12509", "7987"];
       
        var shipMethod = allShipMethods;

        if(shipType == "1"){
            shipMethod = expeditedShipMethods;
        } else if (shipType == "2"){
            shipMethod = groundShipMethods;
        }
        ...
        var filters = [
            ...
            "AND",
            ["custbody_payment_hold_backorder","is","F"] // TODO: CONFIRM WITH PRODUCTION
        ];
        ...
        var inventory_in_gg_bins = search.create({
            ...
            filters: [
                ...
                "AND",
                ["inventorydetail.location", "anyof", "6"], // TODO: Update to Prod
                "AND",
        ...
        var picking_inventory_search = search.create({
            type: "item",
            filters: [
                ["inventorydetail.binnumber", "anyof", "1004"],
                "AND",
                ["inventorydetail.location", "anyof", "6"], // TODO: Prod Values
                "AND",
                ["binonhand.binnumber", "anyof", "1004"]
            ],
            columns: [
                "custitem_sku",
                search.createColumn({
        ...
        log.debug("items_to_fulfill", items_to_fulfill);
        log.debug("picking_inventory", picking_inventory);
        ...
        function cross_reference_inventory(bin_inventory, items_to_fulfill, picking_inventory) {
            ...
            for (var i = 0; i < items_to_fulfill.length; i++) {
                ...
                if (quantity_needed >= 1) { // Fixes partial orders TODO: Discuss with Ryan
