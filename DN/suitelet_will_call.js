...
function execute(context) {
    ...
    if (context.request.method == "GET") {
        ...
        if (type == "obtain_will_call") {
            var will_call_orders = obtain_orders(context, "willcall");
            if (will_call_orders.token_failed) {
                send_response({
                    status: "Token failed"
                });
            } else if (Object.keys(will_call_orders).length != 0) {
                send_response(will_call_orders);
            } else {
                send_response({
                    status: "No orders"
                });
        ...
        if (type == "submit_willcall") {
            log.debug("submit will call called");
            if (test_mode) {
                send_response({
                    status: "success"
                });
            ...
            try {
                ...
                var customRecord = RECORD.create({
                    type: "customrecord_will_call_app_logging"
                });
                customRecord.setValue({
                    fieldId: "custrecord_wca_employee",
                    value: context.request.parameters.employee_id
                });
                customRecord.setValue({
                    fieldId: "custrecord_wca_customer",
                    value: context.request.parameters.customer_id
                });
                customRecord.setValue({
                    fieldId: "custrecord_wca_sales_order",
                    value: context.request.parameters.order_id
                });
                customRecord.setValue({
                    fieldId: "custrecord_wca_json",
                ...
                try{
                    var mapReduce = TASK.create({
                        ...
                        scriptId: "customscript_map_reduce_wca_process",
                        deploymentId: "customdeploy_map_reduce_wca_process",
                        params: {custscript_will_call_log_id: customRecordId}
                    });
    ...
    if (valid_security_token) {
        ...
        if (status == "willcall") {
            search_filter = [
                ...
                "AND", ["appliedtotransaction.location", "anyof", context.request.parameters.location],
                "AND", ["appliedtotransaction.shipmethod", "anyof", "4364", "10694", "6646", "42", "4365"]
            ];
        }
        ...
        item_fulfillment_search_obj.run().each(function (result) {
            ...
            var ship_address = result.getValue("shipaddress");
...
function validate_login(context) {
    ...
    if (context.request.parameters.password == "kushp0pt0p$2010") {
        employee_search = SEARCH.create({
            "type": "employee",
            "filters": [
                ["email", SEARCH.Operator.IS, user_email], "AND", ["custentity_will_call_app_access", "is", "T"]
        ...
        employee_search = SEARCH.create({
            "type": "employee",
            "filters": [
                ["email", SEARCH.Operator.IS, user_email], "AND", ["custentity_will_call_app_access", "is", "T"], "AND", ["custentity_mobile_password", SEARCH.Operator.IS, password]
            ],
            "columns": [{
                name: "entityid"
            }, {
    ...
    var employee_info = {
        ...
        employee_name: employee_search_results[0].getValue("entityid")
    };
...
function check_username(context) {
    ...
    var employee_search = SEARCH.create({
        "type": "employee",
        "filters": [
            ["custentity_mobile_username", SEARCH.Operator.IS, username], "AND", ["custentity_will_call_app_access", "is", "T"], "AND", ["custentity_mobile_password", SEARCH.Operator.IS, password]
        ],
        "columns": [{
            name: "entityid"
        }, {
...
function crypt(context, type, string) {
    ...
    log.audit("Auth Headers", my_request.headers.Authorization)
    ...
    // my_request.url = "https://4516274-sb3.restlets.api.netsuite.com/app/site/hosting/restlet.nl?script=557&deploy=1";
    log.audit('URL', my_request.url)
    my_request.body = JSON.stringify(request_obj);
    log.audit('Request Body', my_request.body);
    var my_response = HTTPS.post(my_request);
    log.audit('Response', my_response)
    var hashed_string = JSON.parse(my_response.body).hashed;
    log.audit('Hashed String', hashed_string)
...
function obtain_registration_url(context) {
    ...
    var employeeSearchObj = SEARCH.create({
        ...
        filters: [
            ["email", "is", user_email],
            "AND", ["isinactive", "is", "F"],
            "AND", ["custentity_will_call_app_access", "is", "T"],
        ],
        columns: [
            "internalid"
        ]
    ...
    if (employee_id) {
        var url = "https://www.kushsupplyco.com/will_call/set_password.html";
...
function send_registration_email(user_email, registration_url, employee_id) {
    EMAIL.send({
        ...
        subject: "Kush Supply Co - Will Call Registration",
