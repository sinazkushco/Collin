<div class="product-details-information-content">
	{{#if showInformation}}
		...
		<!--	{{ name }} -->
			    <h2 class="details_information-title-name">{{name}}</h2>
				<i></i>
				
				{{#if @first}}
				<p class="product-details-information-hint"> {{{trimHtml content 550}}} </p>
				{{/if}}

				{{#unless @first}}
				<p class="product-details-information-hint"> {{{trimHtml content 350}}} </p>
				{{/unless}}
			</button>
		{{/each}}
			...
			<div role="tabpanel">
				...
				<div class="product-details-information-tab-content" data-type="information-content" data-action="tab-content">
					...
					{{#if ../showHeader}}<h2 class="product-details-information-tab-content-panel-title">{{name}}</h2>{{/if}}
						<h2 class="product-details-information-tab-content-panel-title-mobile">{{name}}</h2>
