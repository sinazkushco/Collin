					...
					<table>
						...
						<tbody>
							{{#each priceSchedule}}
								<tr>
									{{#if maximumquantity}}
										<td class="quantity-pricing-table-body-quantity">{{minimumquantity}} &minus; {{maximumquantity}}</td>
										{{#if is_range}}
											<td class="quantity-pricing-table-body-price">{{price_range.min_formatted}} &minus; {{price_range.max_formatted}}</td>
										{{else}}
