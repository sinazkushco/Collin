	...
	{{#if isPriceEnabled}}
		{{#if isPriceRange}}
			<span class="product-views-price-range" itemprop="offers" itemscope itemtype="https://schema.org/AggregateOffer">
				...
				<span class="product-views-price-lead">
					{{translate '<span itemprop="lowPrice" data-rate="$(0)" >$(1)</span> to <span itemprop="highPrice" data-rate="$(2)">$(3)</span>'}}
				</span>
				{{#if showComparePrice}}
		...
		{{else}}
			<span class="product-views-price-exact" itemprop="offers" itemscope itemtype="https://schema.org/Offer">
				...
				<!-- Single -->
				<span class="product-views-price-lead">
					{{#if qantityPrice_FormattedPLP}}
						<span class='lowas-pieces'>As low as</span>
						<div class="qp">
							{{qantityPrice_FormattedPLP}}
						</div>
					{{else if qantityPrice_FormattedPDP}}
						<span class='lowas-pieces'>As low as</span>
						<div class="qp">
							{{qantityPrice_FormattedPDP}}
						</div>
					{{else if perUnitPrice_FormattedPDP}}
						<div class="qp">
							{{perUnitPrice_FormattedPDP}} <span class="lowas-pieces">each</span>
						</div>
						<br>
						<div class="lowas-pieces">
								{{priceFormatted}} ({{units}} piece(s))
						</div>
						
					{{else if perUnitPrice_FormattedPLP}}
						<div class="qp">
							{{perUnitPrice_FormattedPLP}} <span class="lowas-pieces">each</span>
						</div>
						<br>
						<div class="lowas-pieces">
							{{priceFormatted}} ({{units}} piece(s))
